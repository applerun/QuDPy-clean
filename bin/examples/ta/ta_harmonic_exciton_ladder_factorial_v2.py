...
因此 phase grid 同时包含 pump 与 probe 两个 phase axis。
"""
...
def save_case_spectrum_csv(path: Path, *, energy_eV: np.ndarray, projected: np.ndarray) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    values = np.asarray(projected)
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["energy_eV", "projected_real", "projected_imag", "projected_abs"],
        )
        writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    "projected_real": float(np.real(value)),
                    "projected_imag": float(np.imag(value)),
                    "projected_abs": float(np.abs(value)),
                }
            )
    return path
...
def _integrated_abs(energy_eV: np.ndarray, projected: np.ndarray) -> float:
    return float(np.trapz(np.abs(np.asarray(projected)), np.asarray(energy_eV, dtype=float)))
...
def run_one_case(
    ...
    *,
    ...
    ta_plan = TASingleDelayPlan(
        ...
        pump=pump,
        probe=probe,
        delay=TADelayCenters(delay_fs=float(settings.delay_fs), probe_center_fs=0.0),
    ...
    phase_cycling = TAPhaseCyclingSpec(
        ...
        signal_name="pump0_probe1_projected_absorption",
        axis_specs=(
        ...
        normalize=True,
        sign=-1,
        metadata={"meaning": "pump net 0 + probe channel TA-like validation signal"},
    )
    phase_plan = build_ta_pump_probe_phase_cycling_plan(
        ta_plan,
        phase_cycling=phase_cycling,
        case_name=f"ta_factorial_{case.case_name}_phase_cycled",
    )
    ...
    energy = np.asarray(projected_bundle.axes["energy_eV"], dtype=float)
    projected = np.asarray(projected_bundle.projected_signal)
    trace_errors = [
        float(record.single_run_result.dynamics_result.max_trace_error())
        for record in phase_result.case_records
        if record.single_run_result is not None
    ]
    hermiticity_errors = [
        float(record.single_run_result.dynamics_result.max_hermiticity_error())
        for record in phase_result.case_records
        if record.single_run_result is not None
    ]
    spectrum_csv = save_case_spectrum_csv(
        output_dir / "spectra" / f"{case.case_name}_projected_spectrum.csv",
        energy_eV=energy,
        projected=projected,
    )
    ...
    summary = {
        ...
        "max_abs_projected_signal": float(np.max(np.abs(projected))),
        "integrated_abs_projected_signal": _integrated_abs(energy, projected),
...
def maybe_plot_overlay(path: Path, case_results: list[dict[str, Any]]) -> Path | None:
    try:
        import matplotlib.pyplot as plt
    except Exception as exc:
        print(f"Skipping overlay plot because matplotlib import failed: {exc}")
        return None
    ...
    for item in case_results:
        ...
        ax.plot(
            item["energy_eV"],
            np.real(item["projected"]),
            linewidth=1.2,
            label=summary["case_name"],
        )
    ax.axhline(0.0, color="black", linestyle="--", linewidth=0.8, alpha=0.5)
    ax.set_xlim(1.40, 1.80)
    ax.set_xlabel("Probe photon energy (eV)")
    ax.set_ylabel("Projected signal real part")
    ax.set_title("Harmonic exciton ladder factorial projected spectra")
...
def _summary_csv_rows(case_results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    keys = [
        ...
        "pb",
        "eid",
        "max_abs_projected_signal",
        "integrated_abs_projected_signal",
        "n_spectrum_points",
        "energy_min_eV",
        "energy_max_eV",
        "max_trace_error",
...
def run_factorial(args: argparse.Namespace) -> dict[str, Any]:
    ...
    case_results = [
        run_one_case(case, settings=settings, output_dir=output_dir, normalizer=normalizer)
        for case in build_case_config(settings)
    ]
    summary_rows = _summary_csv_rows(case_results)
    summary_csv = save_factorial_summary_csv(output_dir / "factorial_summary.csv", summary_rows)
    overlay_path = None
    if not bool(args.no_plots):
        overlay_path = maybe_plot_overlay(output_dir / "projected_spectra_overlay.png", case_results)
        ...
        "cases": [
            {
                "summary": item["summary"],
                "system": item["system"],
                "base_params_adapter": item["base_params_adapter"],
            }
            for item in case_results
        ],
    }
    meta_path = _write_json(output_dir / "factorial_meta.json", meta)
    ...
    for row in summary_rows:
        print(
            f"{row['case_name']:16s} "
            f"max_abs={row['max_abs_projected_signal']:.6e} "
            f"integrated_abs={row['integrated_abs_projected_signal']:.6e} "
            f"trace={row['max_trace_error']:.3e} "
            f"herm={row['max_hermiticity_error']:.3e}"
        )
    return meta
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite this script's deterministic CSV/JSON/PNG outputs.")
    parser.add_argument("--no-plots", action="store_true", help="Skip overlay PNG.")
    parser.add_argument("--delay-fs", type=float, default=0.0, help="Selected pump-probe delay.")
    parser.add_argument("--eis-eV", type=float, default=-0.02, help="EIS perturbation value when EIS is on.")
    parser.add_argument("--pb", type=float, default=0.7, help="PB perturbation value when PB is on.")
    parser.add_argument("--eid", type=float, default=1.5, help="EID perturbation value when EID is on.")
